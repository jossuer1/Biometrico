...
export default function FaceTecScanner({ onComplete, onCancel }) {
  ...
  async function start() {
    ...
    setError("");
    try {
      const { result, all } = await runIdentitySession({ onStep: setStep });
      console.log("[FaceTec] all:", all);
      onComplete(result);
    } catch (e) {
      setError(e.message || "No se completó la verificación.");
    } finally {
      setBusy(false);
    }
  }
  return (
    <main className="card">
      <p className="eyebrow">Proceso seguro</p>
      ...
      <p>El orden es obligatorio: rostro, frente y reverso.</p>
      {error && <p className="error">{error}</p>}
      <button disabled={busy} onClick={start}>
        {busy ? `Procesando: ${labels[step - 1]}` : "Comenzar con cámara"}
      </button>
